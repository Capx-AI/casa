...
// Local only. Never publishes, activates, or talks to casa.capx.ai.
//
//   node capx/briefs.mjs --brain company-brain
//   node capx/briefs.mjs --brain company-brain --type site --draft
//   node capx/briefs.mjs --brain company-brain --reason "Offer changed"
//
// Casa may copy a local draft under outputs/artifact-change-brief/drafts/.
// Going live is a separate founder step: node capx/publish.mjs ...
...
const USAGE =
  "usage:\n" +
  "  capx/briefs.mjs --brain <dir> [--type site|one-pager|deck] [--draft] [--reason text]\n" +
...
export function writeChangeBrief({ brainDir, type, draft = false, reason = "" } = {}) {
      ...
      if (existsSync(src) && lstatSync(src).isDirectory()) {
        ...
        writeFileSync(
          ...
          [
            ...
            `Do not run capx/publish.mjs or capx/artifacts.mjs activate from the brief.`,
            ``,
          ].join("\n"),
        );
      }
  ...
  const nextStep =
    "Review this brief. If you accept, edit the local draft then run " +
    "node capx/publish.mjs <site|one-pager|deck> --dir <folder> --brain <dir>. " +
    "This command did not publish or activate.";
...
function renderBrief(doc) {
  const lines = [
    ...
    `explicit founder step:`,
    ``,
    `  node capx/publish.mjs <site|one-pager|deck> --dir <folder> --brain company-brain`,
    ``,
  ];
  if (doc.reason) {
